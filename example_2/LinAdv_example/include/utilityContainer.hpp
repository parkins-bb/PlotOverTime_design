...
class BasicElement {
 public:
  ...
  T &operator[](const label i) { return data[i]; }
};

template <class T>
bool compareArray(const std::vector<T> &a, const std::vector<T> &b);
 ...
 public:
  ...
  void display() const {
    for (label i = 0; i < num; ++i) {
      std::cout << "Item: " << i << " (";
      for (label j = startIdx[i]; j < startIdx[i + 1]; ++j) {
        std::cout << data[j];
   ...
   */
  BasicElement<T> &operator[](const label i) {
    if (!basicEle) {
 ...
 */
template <class T>
label partition(std::vector<std::vector<T> > &arr, label l, label r) {
  label k = l, pivot = arr[r][0];
  for (label i = l; i < r; ++i) {
    if (arr[i][0] <= pivot) {
      arr[i].swap(arr[k]);
      k++;
 ...
 */
template <class T>
void quicksortArray(std::vector<std::vector<T> > &arr, label l, label r) {
  if (l < r) {
    label pivot = partition(arr, l, r);
 ...
 */
template <class T>
void filterArray(std::vector<std::vector<T> > &arr, label *faceNum) {
  label num = arr.size();
  quicksortArray(arr, 0, num - 1);
  // for(label i=0;i<tmp;i++) printf("%d\n", arr[0][i]);
  sort(arr[0].begin(), arr[0].end());
  std::vector<std::vector<T> > bndArr, innArr;
  for (label i = 0; i < num; ++i) {
 ...
 */
template <class T>
void transformArray(const std::vector<std::vector<T> > &arr,
                    ArrayArray<T> &res) {
  label cellNum = arr.size();
  res.num = cellNum;
  // 防止内存泄露
  ...
  for (label i = 0; i < cellNum; ++i) {
    label k = 0;
    for (label j = res.startIdx[i]; j < res.startIdx[i + 1]; ++j) {
      res.data[j] = arr[i][k];
      k++;
    }
 ...
 */
template <class T>
void transformArray(const ArrayArray<T> &arr,
  ...
  for (label i = 0; i < cellNum; ++i) {
    ...
    for (label j = arr.startIdx[i]; j < arr.startIdx[i + 1]; ++j) {
 ...
 */
template <class T>
label findArray(std::vector<std::vector<T> > &arr, std::vector<T> &value) {
  label num = arr.size();
  // printf("%d\n", num);
  // printf("*****************************************************\n");
  for (label i = 0; i < num; ++i) {
    // for (label j = 0; j < value.size(); ++j)
    // {
 ...
 */
template <class T>
label findSortedArray(std::vector<std::vector<T> > &arr, std::vector<T> &value,
                      label l, label r) {
...
label sort(std::vector<D> &data,
           const std::vector<W> &weights = std::vector<W>(0)) {
 ...
 */
template <class W>
label ascend(const void *a, const void *b) {
 ...
 */
template <class W>
label descend(const void *a, const void *b) {
  cout << "comparing " << *(W *)a << " and " << *(W *)b << endl;
  if (*(W *)a > *(W *)b)
    return -1;
  else if (*(W *)b < *(W *)a)
...
typedef label ComparePtr(const void *, const void *);
/**
 ...
 */
template <class W>
size_t biSearch(const std::vector<W> &data, W value,
                ComparePtr comp = ascend<W>) {
  // typename std::vector<W>::iterator iter;
