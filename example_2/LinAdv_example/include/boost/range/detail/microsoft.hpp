...
namespace boost { namespace range_detail_microsoft {
    ...
    template< class std::vectorT, class SampleRange >
    bool test_init_array(std::vectorT& arr, SampleRange const& sample)
    {
        ...
        for (iter_t it = boost::const_begin(sample), last = boost::const_end(sample); it != last; ++it) {
            val_t v = *it; // works around ATL3 CSimplestd::vector
            arr.Add(v);
        }
