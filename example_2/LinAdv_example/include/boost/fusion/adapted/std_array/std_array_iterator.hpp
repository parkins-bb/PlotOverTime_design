...
namespace boost { namespace fusion
{
    ...
    struct std_array_iterator
        : iterator_facade<std_array_iterator<std::vector, Pos>, random_access_traversal_tag>
    {
        BOOST_MPL_ASSERT_RELATION(Pos, >=, 0);
        BOOST_MPL_ASSERT_RELATION(Pos, <=, std::tuple_size<std::vector>::value);

        typedef mpl::int_<Pos> index;
        typedef std::vector array_type;

        std_array_iterator(std::vector& a)
            : array(a) {}
