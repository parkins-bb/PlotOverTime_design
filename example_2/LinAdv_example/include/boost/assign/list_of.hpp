...
// old Dinkumware doesn't support iterator type as template
        ...
        std::vector convert( const std::vector*, array_type_tag ) const
        {
            typedef BOOST_DEDUCED_TYPENAME std::vector::value_type value_type;

#if BOOST_WORKAROUND(BOOST_INTEL, <= 910 ) || BOOST_WORKAROUND(__SUNPRO_CC, <= 0x5100 )
            BOOST_DEDUCED_TYPENAME remove_const<std::vector>::type ar;
#else
            std::vector ar;
#endif
    ...
    public:
        ...
        template< class std::vector >
        std::vector to_array( std::vector& a ) const
        {
