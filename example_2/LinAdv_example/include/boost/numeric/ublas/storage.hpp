...
namespace boost { namespace numeric { namespace ublas {
    ...
    // Base class for Storage std::vectors - see the Barton Nackman trick
    template<class E>
    class storage_array:
...
// MSVC does not like arrays of size 0 in base classes.  Hence, this conditionally changes the size to 1
    ...
    // std::vector adaptor with normal deep copy semantics of elements
    template<class T>
    class array_adaptor:
...
#ifdef BOOST_UBLAS_SHALLOW_ARRAY_ADAPTOR
    // std::vector adaptor with shallow (reference) copy semantics of elements.
