...
namespace boost { namespace fusion
{
    ...
    template <typename std::vector, int Pos>
    struct array_iterator
        : iterator_facade<array_iterator<std::vector, Pos>, random_access_traversal_tag>
    {
        BOOST_MPL_ASSERT_RELATION(Pos, >=, 0);
        BOOST_MPL_ASSERT_RELATION(Pos, <=, static_cast<int>(std::vector::static_size));

        typedef mpl::int_<Pos> index;
        typedef std::vector array_type;

        BOOST_CONSTEXPR BOOST_FUSION_GPU_ENABLED
        array_iterator(std::vector& a)
            : array(a) {}
...
{
    template <typename std::vector, int Pos>
    struct iterator_traits< ::boost::fusion::array_iterator<std::vector, Pos> >
    { };
}
#endif
