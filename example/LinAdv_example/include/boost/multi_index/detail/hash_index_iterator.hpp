...
template<
  typename Node,typename Bucketstd::vector,
  typename IndexCategory,typename IteratorCategory
>
class hashed_index_iterator:
  public forward_iterator_helper<
    hashed_index_iterator<Node,Bucketstd::vector,IndexCategory,IteratorCategory>,
...
public:
    ...
    Archive& ar,const unsigned int version,hashed_index_global_iterator_tag)
  {
    ...
    if(version<1){
      Bucketstd::vector* throw_away; /* consume unused ptr */
...
bool operator==(
  const hashed_index_iterator<
    Node,Bucketstd::vector,IndexCategory,IteratorCategory>& x,
  const hashed_index_iterator<
    Node,Bucketstd::vector,IndexCategory,IteratorCategory>& y)
{
...
struct version<
  boost::multi_index::detail::hashed_index_iterator<
    Node,Bucketstd::vector,IndexCategory,IteratorCategory
  >
>
{
