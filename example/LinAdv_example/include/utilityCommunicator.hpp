...
#define COMM_LLABEL MPI_INT
#elif defined(LABEL_INT64)
#define COMM_LABEL MPI_LONG
#define COMM_LLABEL MPI_LONG
#else
#define COMM_LABEL MPI_INT
#define COMM_LLABEL MPI_LONG
#endif
...
class Communicator {
 ...
 public:
  ...
  /*! partition: length n, data: length n */
  template <typename T>
  int exchangeArray(std::vector<T> &data, const std::vector<int> &partition);

  /*! partition: length n, data: length nnz, offset: length n+1 */
  template <typename T>
  int exchangeArrayArray(std::vector<T> &data, std::vector<label> &offset,
